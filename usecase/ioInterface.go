...
type IDataIO interface {
	...
	AddTheme(themeName string) (*forum.ThemeInDB, error) //新增主题
	DeleteTheme(themeID int64) error                     //删除主题
	UpdateTheme(theme *forum.ThemeInDB) error            //更新主题名称
	QueryTheme(themeID int64) (*forum.ThemeInDB, error)  //查询主题
	QueryThemes() ([]*forum.ThemeInDB, error)            //查询主题列表

	AddPost(post *forum.PostInDB) error                                                      //新增帖子
	AddPosts(post []*forum.PostInDB) error                                                   //批量新增帖子
	DeletePost(postID int64) error                                                           //删除帖子
	QueryPostCountOfTheme(themeID int64) int                                                 //查询主题帖子总数量
	QueryPosts(themeID int64, count, offset, sortType int) ([]*forum.PostOnThemePage, error) //查询主题下的帖子列表
	QueryPost(postID int64) (*forum.PostInDB, error)                                         //查询DB帖子
	QueryPostPG(postID int64) (*forum.PostOnPostPage, error)                                 //查询帖子页内，帖子的展示内容
	...
	QueryPgComments(postID int64) ([]*forum.CmtOnPostPage, error) //查询帖子页内，评论的展示内容
