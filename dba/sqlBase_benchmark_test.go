...
func BenchmarkInsertComment(b *testing.B) {
	...
	cmts := make([]*forum.CommentInDB, 0, 200)
...
func BenchmarkBatchInsertComment(b *testing.B) {
	...
	//多轮评论
	for i := 0; i < b.N; i++ {
		cmts := make([]*forum.CommentInDB, 0, 500)
		for i := 0; i < 500; i++ {
...
func BenchmarkQueryPostPage(b *testing.B) {
	...
	maxPostCount := 1000
	tmPostMap := make(map[int][]*forum.PostOnThemePage, 16)
	for _, v := range tms {
		tmPostMap[v.ID] = make([]*forum.PostOnThemePage, 0, maxPostCount)
