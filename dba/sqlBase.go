...
//QueryTheme 查询某个主题
func (s *sqlBase) QueryTheme(themeID int) (*forum.ThemeInDB, error) {
	tm := &forum.ThemeInDB{ID: themeID}
...
//AddPost 新增帖子
func (s *sqlBase) AddPost(post *forum.PostInDB, cmt *forum.CommentInDB) error {
	tx, _ := s.Begin()
...
//AddPosts 批量新增帖子
func (s *sqlBase) AddPosts(posts []*forum.PostInDB, cmts []*forum.CommentInDB) error {
	tx, _ := s.Begin()
...
//QueryPost 查询帖子
func (s *sqlBase) QueryPost(postID int) (*forum.PostInDB, error) {
...
func (s *sqlBase) UpdatePostTitle(post *forum.PostInDB) error {
	tx, _ := s.Begin()
...
func (s *sqlBase) QueryPostsOfTheme(themeID int, count, offset, sortType int) ([]*forum.PostOnThemePage, error) {
	var rows *sql.Rows
	var err error
	if sortType == 0 {
...
func (s *sqlBase) QueryPostsOfUser(userID int, count, offset int) ([]*forum.PostOnThemePage, error) {
...
func (s *sqlBase) QueryPostOfPostPage(postID int) (*forum.PostOnPostPage, error) {
...
//QueryComment 查询单个评论
func (s *sqlBase) QueryComment(cmtID int) (*forum.CommentInDB, error) {
...
func (s *sqlBase) UpdateComment(cmt *forum.CommentInDB) error {
	tx, _ := s.Begin()
...
func (s *sqlBase) QueryComments(postID int) ([]*forum.CommentInDB, error) {
	rows, err := s.Query(sqlStrToQueryComments, postID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	cmts := make([]*forum.CommentInDB, 0, 32)
	for rows.Next() {
		cmt := new(forum.CommentInDB)
		err = rows.Scan(
			&cmt.ID,
...
func (s *sqlBase) QueryCommentsOfPostPage(postID int, count int, offset int, userID int) ([]*forum.CmtOnPostPage, error) {
	cmts := make([]*forum.CmtOnPostPage, 0, count)
	...
	defer rowspb.Close()
	f := func(id int) *forum.CmtOnPostPage {
		for _, v := range cmts {
			if v.ID == id {
				return v
			}
...
//AddPbItem 新增赞踩行
func (s *sqlBase) AddPbItem(pb *forum.PBInDB) error {
...
//QueryPbItem 查询赞踩行
func (s *sqlBase) QueryPbItem(cmtID int, userID int) (*forum.PBInDB, error) {
	var pbIns = new(forum.PBInDB)
	pbIns.CmtID = cmtID
	pbIns.UserID = userID
...
//Praise 赞
func (s *sqlBase) Praise(pb *forum.PBInDB) error {
	if pb.PValue == 1 {
		return nil
	}
	tx, _ := s.Begin()
...
//Belittle 贬
func (s *sqlBase) Belittle(pb *forum.PBInDB) error {
	if pb.BValue == 1 {
		return nil
	}
	tx, _ := s.Begin()
...
//PraiseCancel 取消赞
func (s *sqlBase) PraiseCancel(pb *forum.PBInDB) error {
	if pb.PValue == 0 {
		return nil
	}
	tx, _ := s.Begin()
...
//BelittleCancel 取消贬
func (s *sqlBase) BelittleCancel(pb *forum.PBInDB) error {
	if pb.BValue == 0 {
		return nil
	}
	tx, _ := s.Begin()
...
//QueryUserByID 查询用户
func (s *sqlBase) QueryUserByID(userID int) (*forum.UserInDB, error) {
